...
fname = join(args.log_dir, 'hdgm' + '.txt')
...
for kk in range(K):
    ...
    for k in range(N):
        ...
        h_u, threshold_u, jsv_u = JSV_Gaussian(S, N_per, n*Num_clusters, alpha, n_components)
        # Gather results
        count_u = count_u + h_u
        # if k % 10 == 0:
...
with open(fname, 'a') as f:
    f.write("HDGM_" + args.exptype + "_" + str(n) + str(d)+ "_" + "_ncomp_" + str(n_components) + "_GMM:" + str(Results) + "_" + str(np.mean(Results[0])) + "_" + str(np.std(Results[0])) + '\n')