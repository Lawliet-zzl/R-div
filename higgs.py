...
def evaluate():
	...
	for rep in range(100):
		...
		datam = np.concatenate([data1[:max_data_size], data2[:max_data_size]], axis=0)
		logprob_1 = get_kde_estimates(bandwidth, datam, data1)
		logprob_2 = get_kde_estimates(bandwidth, datam, data2)
		vdiv = abs(logprob_1 - logprob_2)
...
def get_scores():	
	...
	for run in range(args.runs):
		...
		with open(fname, 'a') as f:
			f.write(str(max_data_size) + " " + str(np.mean(test_power_val)) + " " + str(np.std(test_power_val)) + '\n')
