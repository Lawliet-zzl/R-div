...
fname = join(args.log_dir, 'blob_kde' + '.txt')
...
for n in n_list:
    ...
    for kk in range(K):
        ...
        for k in range(N):
            ...
            h_u, threshold_u, jsv_u = JSV_KDE(S, N_per, N1, alpha, bandwidth)
            # Gather results
            count_u = count_u + h_u
            H_u[k] = h_u
            T_u[k] = threshold_u
    ...
    with open(fname, 'a') as f:
        f.write("Blob_" + args.exptype + "_" + str(n)+ "_gaussian_" + "_KDE:" + str(Results) + '\n')
